...
```{r setup}
reticulate::use_python("/home/jarning/miniconda3/envs/test-env/bin/python") # for leiden
...
seu = qs::qread("CosMx_data/CosMx_Lung5_Rep1.SeuratV5.v1.qs") # Macrophage-1, Macrophage-2
...
## stack plot
row.order = c("Tumor", "LTF+ Epi", "CAF", "Endo", "Macrophage-1", "Macrophage-2", "Neutrophil", "Mast", "Plasma", "B cell", "T cell")
niche.average = niche.average[row.order, ]

data.plot = niche.average %>% as.data.frame() %>% 
  mutate(niche = factor(rownames(.), levels = row.order)) %>% 
  pivot_longer(cols = 1:6, names_to = "CAF.sub", values_to = "Fraction")
