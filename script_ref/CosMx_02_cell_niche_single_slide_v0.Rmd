...
## 思考：两种注释的结果有什么不同？为什么？
# seu = qs::qread("CosMx_data/CosMx_Lung5_Rep1.SeuratV5.v0.qs") # 合并Macrophage注释
...
## stack plot
row.order = c("Tumor", "LTF+ Epi", "CAF", "Endo", "Macrophage", "Neutrophil", "Mast", "Plasma", "B cell", "T cell")
niche.average = niche.average[row.order, ]

data.plot = niche.average %>% as.data.frame() %>% 
  mutate(niche = factor(rownames(.), levels = row.order)) %>% 
  pivot_longer(cols = 1:4, names_to = "CAF.sub", values_to = "Fraction")
...
```{r}
source("../utils/R/io.R")
slim_save(seu.caf, file = "CosMx_data/CosMx_Lung5_Rep1_CAF.SeuratV5.qs")
```
