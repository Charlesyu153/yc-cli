...
本文档提供 YC-CLI 中三个 AI Agents 的详细使用指南和示例。

---

## 概述

YC-CLI 提供三个专用 AI Agent，用于不同的开发场景：

| Agent | 角色 | 主要职责 |
|-------|------|----------|
| code-architect | 代码架构师 | 架构设计、技术选型、实施规划 |
| code-explorer | 代码探索者 | 代码搜索、结构分析、文档生成 |
| code-reviewer | 代码审查者 | 质量检查、问题发现、改进建议 |
...
- `agents/agents.json` - 主配置文件
- `agents/code-architect.json` - 架构师配置
- `agents/code-explorer.json` - 探索者配置
- `agents/code-reviewer.json` - 审查者配置
...
**提示语**：
```
请以 code-architect 的角色，帮我重构 src/data_processor.py
...
**提示语**：
```
请以 code-explorer 的角色，帮我查找所有处理用户认证的代码。

需要：
1. 查找包含 "auth" 或 "authentication" 关键字的所有文件
2. 定位关键函数的位置（文件:行号）
3. 分析这些代码的功能
4. 总结认证相关的处理流程
```
...
**提示语**：
```
请以 code-explorer 的角色，帮我理解数据处理的完整分析流程。

请分析：
1. scripts/analysis/ 目录下的所有脚本
2. 脚本的执行顺序和依赖关系
3. 每个脚本的主要功能
4. 输入输出数据
5. 关键函数和参数
...
**提示语**：
```
请以 code-explorer 的角色，追踪 process_data() 函数的调用链。
...
**提示语**：
```
请以 code-reviewer 的角色，审查 src/data_processor.py
...
**提示语**：
```
请以 code-reviewer 的角色，审查 scripts/analysis/ 目录下的所有 Python 脚本。

重点检查：
1. 哪些文件超过 500 行（需要重构）
2. 代码规范符合性（PEP 8、命名规范）
3. 共性问题和改进建议
4. 优先级排序（哪些问题最需要修复）
```
...
**问题**：某个功能处理失败，需要快速定位问题

**步骤**：
```
1. 使用 code-explorer 查找相关的处理代码
   提示语：请以 code-explorer 的角色，查找处理 [功能名] 的所有相关代码
...
- **Skills 优先**：优先使用可用的 Skills 来完成任务
...
所有 agents 都会遵循项目规范（详见 `docs/STANDARDS.md`）：
- 禁止使用表情符号
- 代码文件不超过 500 行
- 遵循编码规范（Python PEP 8、R 规范）
- 每完成一步就 commit
- 任务完成后归档文档
...
- 主配置: `agents/agents.json`
- 架构师: `agents/code-architect.json`
- 探索者: `agents/code-explorer.json`
- 审查者: `agents/code-reviewer.json`
- 项目规范: `config/standards.json`
- 使用文档: `docs/AGENTS-GUIDE.md`（本文档）
