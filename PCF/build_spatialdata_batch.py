...
用法：
    python build_spatialdata_batch.py
...
def annotate_cell(row: pd.Series, marker_intensity_cols: list, vimentin_observable: bool) -> dict:
    ...
    if total_intensity < NEGATIVE_THRESHOLD:
        ...
        result["annotation"] = "Negative_or_Artifact"
        return result
    ...
    if pos("PanCK") and not pos("CD31"):
        ...
        if pos("CLDN18.2"):
            ...
            result["annotation"] = "Tumor_CLDN18p"
        else:
            prefix = "Tumor_CLDN18n"
            result["annotation"] = "Tumor_CLDN18n"
        
        # PYCR1 分层（仅用于 cell_type_lvl2）
        if pos("PYCR1"):
    ...
    if not pos("PanCK") and not pos("CD31") and not has_immune_marker:
        ...
        if pos("Vimentin"):
            ...
            result["annotation"] = "Stromal_other"
            _add_common_states(result, row, pos)
            return result
...
def _select_obs_columns(df: pd.DataFrame, marker_cols: set[str]) -> list[str]:
    ...
    for c in df.columns:
        ...
        if c.startswith("bool_") or c in {"major_lineage", "cell_type_lvl1", "cell_type_lvl2", "annotation"} or c.startswith("st_"):
            keep.append(c)
            continue
