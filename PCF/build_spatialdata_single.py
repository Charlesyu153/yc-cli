#!/usr/bin/env python3
"""
单样本测试脚本：读取 cleandata 下某个样本的 objects.tsv，
构建 AnnData / SpatialData 对象，并基于 marker 规则进行细胞谱系与状态注释。

用法：
    python build_spatialdata_single.py

输出：
    - spatial/<sample_id>.annotated.h5ad
    - spatial/<sample_id>.sdata.zarr
"""
...
def annotate_cell(row: pd.Series, marker_intensity_cols: list) -> dict:
    """
    ...
    -------
    dict
        包含 major_lineage, cell_type_lvl1, cell_type_lvl2, 及各 st_* 状态标签
    """
    result = {
    ...
    # ========================================================================
    # 2. Endothelial（血管内皮）: CD31+
    # ========================================================================
    if pos("CD31"):
        result["major_lineage"] = "Endothelial"
    ...
    if pos("PanCK") and not pos("CD31"):
        ...
        # PYCR1 分层
        if pos("PYCR1"):
            result["cell_type_lvl2"] = f"{prefix}_PYCR1p"
        else:
            result["cell_type_lvl2"] = f"{prefix}_PYCR1n"
    ...
    # ========================================================================
    # 5. Stromal / Mesenchymal: Vimentin+ AND PanCK- AND CD31- 且不满足免疫条件
    # ========================================================================
    if pos("Vimentin") and not pos("PanCK") and not pos("CD31") and not has_immune_marker:
        result["major_lineage"] = "Stromal"
        result["cell_type_lvl1"] = "Stromal_VIM+"
        
        if pos("Vimentin") and pos("SMA") and not pos("CD31"):
            result["cell_type_lvl2"] = "CAF"
            # CAF 状态
            if pos("PD-L1"):
        ...
        elif pos("Vimentin") and not pos("SMA") and not pos("CD31"):
            result["cell_type_lvl2"] = "Fibroblast_other"
        elif pos("Vimentin") and pos("SMA") and pos("CD31"):
            result["cell_type_lvl2"] = "Perivascular_or_pericyte"
        else:
...
def annotate_dataframe(df: pd.DataFrame, marker_config: dict) -> pd.DataFrame:
    ...
    annotations = df.apply(
        lambda row: annotate_cell(row, marker_intensity_cols), 
        axis=1, 
        result_type="expand"
    )
...
def _select_obs_columns(df: pd.DataFrame, marker_cols: set[str]) -> list[str]:
    ...
    for c in df.columns:
        ...
        # 由 clean 脚本生成的 positivity
        if c.startswith(POSITIVITY_PREFIX):
            keep.append(c)
            continue

        # 由本脚本生成的 bool/注释列
        if c.startswith("bool_") or c in {"major_lineage", "cell_type_lvl1", "cell_type_lvl2"} or c.startswith("st_"):
            keep.append(c)
            continue

        # 显式剔除常见的大体积 MX 通道列（不同样本会有带/不带 resolution 的两种格式）
        if re.match(r"^MX\s*-\s*.*(resolution\s*#\d+\s*-\s*)?(Red|Green|Blue|Hematoxylin)\s*$", c, flags=re.IGNORECASE):
            continue

        # 其他列默认不写入 obs（避免无意间膨胀输出体积）
        continue
...
def process_single_sample(sample_dir: Path, outdir: Path) -> tuple:
    """
    处理单个样本。
    ...
    -------
    tuple
        (AnnData, SpatialData) 或 (None, None) 如果处理失败
    """
    ...
    # 6. 构建 SpatialData
    sdata = build_spatialdata_from_adata(adata, sample_id)
    
    # 7. 保存输出
    outdir.mkdir(parents=True, exist_ok=True)
    
    h5ad_path = outdir / f"{sample_id}.annotated.h5ad"
    zarr_path = outdir / f"{sample_id}.sdata.zarr"
    ...
    logger.info(f"已保存 SpatialData: {zarr_path}")
    
    return adata, sdata


def main(cleandata_root: str = None, outdir: str = None, sample_id: str = "1P"):
    """
    主入口函数（单样本测试）。
    ...
    sample_dir = cleandata_root / sample_id
    
    if not sample_dir.exists():
        raise FileNotFoundError(f"样本目录不存在: {sample_dir}")
    
    adata, sdata = process_single_sample(sample_dir, outdir)
    
    if adata is not None:
        logger.info("=" * 60)
        logger.info("处理完成！")
        logger.info(f"细胞总数: {adata.n_obs}")
        logger.info(f"Marker 数: {adata.n_vars}")
        logger.info("=" * 60)
