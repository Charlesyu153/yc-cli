...
compute_corrected_niche <- function(niche_mat, niche_mat_norm) {
  section_counts <- colSums(niche_mat)
  total <- sum(section_counts)
  if (total == 0) {
    section_prop <- rep(NA_real_, ncol(niche_mat))
    names(section_prop) <- colnames(niche_mat)
  } else {
    section_prop <- section_counts / total
  }
  section_prop[section_prop == 0] <- NA_real_
  corrected <- sweep(niche_mat_norm, 2, section_prop, "/")
...
input_dir <- opts$input %||% file.path(root_dir, "data")
output_dir <- opts$output %||% file.path(root_dir, "data", "caf_niche")
...
for (file in input_files) {
  ...
  niche_mat <- niche_res$niche.mat
  niche_mat_norm <- niche_res$niche.mat.norm
  niche_mat_correct <- compute_corrected_niche(niche_mat, niche_mat_norm)
